	...
	Object.keys(componentProps).forEach((prop) => {
		...
		if (prop === 'keyframes') {
			...
			Object.entries(propValue as object).forEach(([key, value]) => {
				...
				Object.entries(value).forEach(([subKey, subValue]) => {
					...
					Object.entries(subValue as object).forEach(([prop, propValue]) => {
						const cssProperty = cssPropertyMappings[prop as keyof typeof cssPropertyMappings];
			...
			Object.keys(propValue as object).forEach((afterProp) => {
				...
				const cssProperty = cssPropertyMappings[afterProp as keyof typeof cssPropertyMappings];
				const mapProperty = theme[cssProperty as keyof typeof theme];
				...
				if (typeof afterPropValue === 'string') {
					...
					if (mapProperty !== undefined) {
						value = mapProperty[propValueWithBreakpoint as keyof typeof mapProperty];
					}
			...
			Object.keys(propValue as object).forEach((beforeProp) => {
				...
				const cssProperty = cssPropertyMappings[beforeProp as keyof typeof cssPropertyMappings];
				const mapProperty = theme[cssProperty as keyof typeof theme];
			...
			Object.keys(propValue as object).forEach((hoverProp) => {
				...
				const cssProperty = cssPropertyMappings[hoverProp as keyof typeof cssPropertyMappings];
				const mapProperty = theme[cssProperty as keyof typeof theme];
